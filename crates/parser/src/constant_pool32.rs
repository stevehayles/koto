...
impl ConstantPool {
    ...
    /// Returns the f32 corresponding to the provided constant index
    ///
    /// Warning! Panics if there isn't an f32 at the provided index
    pub fn get_float(&self, index: ConstantIndex) -> f32 {
        self.get_f32(index)
    }
    ...
    ///
    /// Warning! Panics if there isn't an f32 at the provided index
    pub fn get_f32(&self, index: ConstantIndex) -> f32 {
        match self.constants.get(usize::from(index)) {
    ...
    /// Returns the i32 corresponding to the provided constant index
    ///
    /// Warning! Panics if there isn't an i32 at the provided index
    pub fn get_int(&self, index: ConstantIndex) -> i32 {
        self.get_i32(index)
    }
    ...
    ///
    /// Warning! Panics if there isn't an i32 at the provided index
    pub fn get_i32(&self, index: ConstantIndex) -> i32 {
        match self.constants.get(usize::from(index)) {
...
impl ConstantPoolBuilder {
    ...
    pub fn add_f32(&mut self, n: f32) -> Result<ConstantIndex, InternalError> {
        let n_u32 = n.to_bits();

        match self.float_map.get(&n_u32) {
            Some(index) => Ok(*index),
            None => {
                let result = ConstantIndex::try_from(self.constants.len())
                    .map_err(|_| InternalError::ConstantPoolCapacityOverflow)?;
                self.constants.push(ConstantEntry::KFloat(n));
                n_u32.hash(&mut self.hasher);
                self.float_map.insert(n_u32, result);
                Ok(result)
            }
        }
    }
    ...
    pub fn add_i32(&mut self, n: i32) -> Result<ConstantIndex, InternalError> {
...
mod tests {
    ...
    fn test_adding_numbers() {
        ...
        assert_eq!(ConstantIndex(0), builder.add_i32(n1).unwrap());
        assert_eq!(ConstantIndex(1), builder.add_f32(n2).unwrap());
        ...
        assert_eq!(n1, pool.get_i32(0.into()));
        assert!(floats_are_equal(n2, pool.get_f32(1.into())));
    ...
    fn test_adding_numbers_and_strings() {
        ...
        assert_eq!(ConstantIndex(0), builder.add_f32(n1).unwrap());
        assert_eq!(ConstantIndex(1), builder.add_string(s1).unwrap());
        assert_eq!(ConstantIndex(2), builder.add_i32(n2).unwrap());
        assert_eq!(ConstantIndex(3), builder.add_string(s2).unwrap());

        let pool = builder.build();

        assert!(floats_are_equal(n1, pool.get_f32(0.into())));
        assert_eq!(s1, pool.get_str(1.into()));
        assert_eq!(n2, pool.get_i32(2.into()));
    ...
    fn test_iter() {
        ...
        builder.add_i32(n1).unwrap();
        builder.add_string(s1).unwrap();
        builder.add_f32(n2).unwrap();
